...
// lib/game/plugins/button.js
ig.baked=true;ig.module('game.plugins.button').requires('impact.game').defines(function(){EntityButton=ig.Entity.extend({size:{x:80,y:40},text:[],textPos:{x:5,y:5},textAlign:ig.Font.ALIGN.LEFT,font:null,animSheet:null,state:'idle',_oldPressed:false,_startedIn:false,_touchActionName:'click',_keyActionName:'click',init:function(x,y,settings){this.parent(x,y,settings);this._touchActionName=settings.touchName;this._keyActionName=settings.keyName;},update:function(){if(this.state!=='hidden'){var _clicked=ig.input.state(this._touchActionName)||ig.input.state(this._keyActionName);if(!this._oldPressed&&_clicked&&(this._inButton()||ig.input.state(this._keyActionName))){this._startedIn=true;}
if(this._startedIn&&this.state!=='deactive'&&(this._inButton()||ig.input.state(this._keyActionName))){if(_clicked&&!this._oldPressed){this.setState('active');this.pressedDown();}
...
this._oldPressed=_clicked;}},draw:function(){},setState:function(s){this.state=s;},pressedDown:function(){console.log('pressedDown');},pressed:function(){console.log('pressed');},pressedUp:function(){console.log('pressedUp');},_inButton:function(){return ig.input.mouse.x+ig.game.screen.x>this.pos.x&&ig.input.mouse.x+ig.game.screen.x<this.pos.x+this.size.x&&ig.input.mouse.y+ig.game.screen.y>this.pos.y&&ig.input.mouse.y+ig.game.screen.y<this.pos.y+this.size.y;}});});

// lib/game/entities/booster.js
ig.baked=true;ig.module('game.entities.booster').requires('impact.game','game.plugins.button').defines(function(){EntityBooster=EntityButton.extend({anim:null,zIndex:1,init:function(x,y,settings){this.parent(x,y,settings);var animSheet=new ig.AnimationSheet(settings.imageName,100,100);this.anim=new ig.Animation(animSheet,0.1,[0]);},update:function(){if(ig.gameStatus===ig.gameStates.flying){this.parent();}
this.anim.update();},pressed:function(){this.parent();ig.rocketDirection=this.rocketDirection;},draw:function(){this.anim.draw(this.pos.x,this.pos.y);}});});

// lib/game/entities/rocket.js
ig.baked=true;ig.module('game.entities.rocket').requires('impact.game','impact.font').defines(function(){EntityRocket=ig.Entity.extend({anim:null,_horizontalSpeed:2,zIndex:100,fuel:100.00,fuelTimer:new ig.Timer(0.75),fuelFont:new Font('15px Garamond'),fuelColor:'green',points:0,pointsTimer:new ig.Timer(0.075),size:{x:41,y:88},shakingPos:{x:0,y:0},checkAgainst:ig.Entity.TYPE.BOTH,init:function(x,y,settings){this.parent(x,y,settings);var animSheet=new ig.AnimationSheet(settings.imageName,41,88);this.anim=new ig.Animation(animSheet,0.1,[0]);},update:function(){this.parent();if(ig.rocketDirection!==false){if(ig.rocketDirection==="left"){this.pos.x-=this._horizontalSpeed;if(this.pos.x<0){this.pos.x=0;}}else if(ig.rocketDirection==="right"){this.pos.x+=this._horizontalSpeed;if(this.pos.x+41>ig.system.width){this.pos.x=ig.system.width-41;}}}
ig.rocketDirection=false;if(ig.gameStatus==ig.gameStates.launchRocket){this.accel.y=-ig.rocketVerticalAcceleration;if(this.pos.y<=350){rollingBackground=ig.game.getEntitiesByType("EntityRollingBackground")[0];rollingBackground.accel.y=-this.accel.y;rollingBackground.vel.y=-this.vel.y;ig.gameStatus=ig.gameStates.flying;this.accel.y=0;this.vel.y=0;this.pos.y=350;ig.game.getEntitiesByType("EntityMessageManager")[0].pushMessage("Captn! You've got control",3);}}
if((ig.gameStatus==ig.gameStates.launchRocket||ig.gameStatus==ig.gameStates.flying)&&this.fuelTimer.delta()>=0){this.fuel--;this.fuelTimer.reset();if(this.fuel<=0){ig.gameStatus=ig.gameStates.gameEnded;ig.rocketBoosterSound.stop();ig.explosionSound.play();this.fuel=100;this.points=0;this.pos.y=468;this.fuelColor='green';}else if(this.fuel<20){this.fuelColor='red';}else if(this.fuel<50){this.fuelColor="yellow";}else{this.fuelColor='green';}}
if((ig.gameStatus==ig.gameStates.launchRocket||ig.gameStatus==ig.gameStates.flying)&&this.pointsTimer.delta()>=0){this.points++;this.pointsTimer.reset();}
if(ig.gameStatus==ig.gameStates.launchRocket||ig.gameStatus==ig.gameStates.flying){this.calculateShaking();}},check:function(other){if(other instanceof EntityFuelRocket){this.fuel+=45;other.kill();}else if(other instanceof EntityAsteroid){if(ig.gameStatus==ig.gameStates.flying){ig.gameStatus=ig.gameStates.gameEnded;ig.rocketBoosterSound.stop();ig.explosionSound.play();this.fuel=100;this.points=0;this.pos.y=468;this.fuelColor='green';}}},calculateShaking:function(){var rand=Math.random();if(ig.gameStatus==ig.gameStates.flying){if(rand<0.33){this.shakingPos.y=2;}else if(rand<0.66){this.shakingPos.y=1;}else{this.shakingPos.y=0}
if(Math.random()<0.5){this.shakingPos.y*=-1}
rand=Math.random();if(rand<0.33){this.shakingPos.x=2;}else if(rand<0.66){this.shakingPos.x=1;}else{this.shakingPos.x=0}
if(Math.random()<0.5){this.shakingPos.x*=-1}}else{if(rand<0.5){this.shakingPos.y=1;}else{this.shakingPos.y=0;}
if(rand<0.5){this.shakingPos.x=1;}else{this.shakingPos.x=0;}}},draw:function(){this.anim.draw(this.pos.x-this.shakingPos.x,this.pos.y-this.shakingPos.y);this.fuelFont.draw('Fuel: '+this.fuel+"%",5,55,'left',this.fuelColor);this.fuelFont.draw('Points: '+this.points,5,75,'left',"white");}});});

// lib/game/entities/rollingBackground.js
ig.baked=true;ig.module('game.entities.rollingBackground').requires('impact.game').defines(function(){EntityRollingBackground=ig.Entity.extend({anim:null,zIndex:0,maxVel:{x:500,y:500},init:function(x,y,settings){this.parent(x,y,settings);var animSheet=new ig.AnimationSheet(settings.imageName,800,10000);this.anim=new ig.Animation(animSheet,0.1,[0]);this.accel.y=0;this.vel.y=0;},update:function(){this.parent();if(this.pos.y>570){ig.game.getEntitiesByType(EntityMessageManager)[0].pushMessage("You are in black space...",10);}},draw:function(){this.anim.draw(this.pos.x,this.pos.y);},reboot:function(){this.vel.y=0;this.accel.y=0;this.pos.y=-9282;}});});

// lib/game/plugins/font.js
ig.baked=true;ig.module('game.plugins.font').requires('impact.impact').defines(function(){Font=ig.Class.extend({align:'left',alpha:1,baseline:'top',colors:['#FFFFFF'],current:0,flicker:0.15,font:null,pos:{x:0,y:0},size:0,text:'',vel:{x:0,y:0},init:function(font,x,y,settings){this.flicker=new ig.Timer(this.flicker);this.font=font||'20px Garamond';this.pos.x=x||0;this.pos.y=y||0;this.size=this.getSize();ig.merge(this,settings);},draw:function(text,x,y,align,color){var ctx=ig.system.context;if(!ig.game._font||ig.game._font!==this.font){ig.game._font=this.font;ctx.font=this.font;}
ctx.save();ctx.globalAlpha=this.alpha;ctx.textAlign=align||this.align;ctx.textBaseline=this.baseline;ctx.fillStyle=color||this.colors[this.current];ctx.fillText(text||this.text,ig.system.getDrawPos((x||this.pos.x)),ig.system.getDrawPos((y||this.pos.y)));ctx.restore();},getSize:function(font){return Number(/\d+/.exec(font||this.font));},getWidth:function(text){return Font.Width(text||this.text,this.font);},update:function(){this.pos.x+=this.vel.x*ig.system.tick;this.pos.y+=this.vel.y*ig.system.tick;if(this.flicker.delta()>0){if(++this.current===this.colors.length){this.current=0;}
this.flicker.reset();}}});Font.Width=function(text,font){var ctx=ig.system.context;if(!ig.game._font||font&&ig.game._font!==font){ig.game._font=font;ctx.font=font;}
return ctx.measureText(text).width/ig.system.scale;};});

// lib/game/entities/messageManager.js
ig.baked=true;ig.module('game.entities.messageManager').requires('impact.game','impact.font','game.plugins.font').defines(function(){EntityMessageManager=ig.Entity.extend({zIndex:50,font:new Font('20px Garamond'),fontCountDown:new Font('40px Garamond'),messageTimer:new ig.Timer(2),messageText:"",init:function(x,y,settings){this.parent(x,y,settings);},update:function(){this.parent();},draw:function(){if(ig.gameStatus===ig.gameStates.launchPrep){this.font.draw('Launch in:',160,220,'center','#FF0000');if(ig.timeTillLaunch==0){this.fontCountDown.draw("GO",160,250,'center','#FF0000');}else{this.font.draw(ig.timeTillLaunch,160,250,'center','#FF0000');}}else if(this.messageText!==""){if(this.messageTimer.delta()<0){this.font.draw(this.messageText,160,250,'center','#FF0000');}else{this.messageText="";}}else{this.messageText="";}},pushMessage:function(messageTxt,messageLifeTime){this.messageText=messageTxt;this.messageTimer.set(messageLifeTime);},setText:function(messageTxt){this.messageText=messageTxt;}});});

// lib/game/entities/radar.js
ig.baked=true;ig.module('game.entities.radar').requires('impact.game').defines(function(){EntityRadar=ig.Entity.extend({anim:null,animRadarScan:null,animBlipScan:null,posRadarScan:{x:0,y:0},blipPositionX:-50,shakingPos:{x:0,y:0},init:function(x,y,settings){this.parent(x,y,settings);var animSheet=new ig.AnimationSheet(settings.imageName,800,50);this.anim=new ig.Animation(animSheet,0.1,[0]);animSheet=new ig.AnimationSheet('media/radarScan.png',50,50);this.animRadarScan=new ig.Animation(animSheet,0.1,[0]);animSheet=new ig.AnimationSheet('media/notAsteroid.png',10,10);this.animBlipScan=new ig.Animation(animSheet,0.1,[0]);},update:function(){this.parent();this.posRadarScan.x+=2;if(this.posRadarScan.x>ig.system.width+50){this.posRadarScan.x=-50;}
if(ig.gameStatus==ig.gameStates.launchRocket||ig.gameStatus==ig.gameStates.flying){this.calculateShaking();}},draw:function(){this.anim.draw(this.pos.x,this.pos.y);this.animRadarScan.draw(this.posRadarScan.x-this.shakingPos.x,this.posRadarScan.y-this.shakingPos.y);this.animBlipScan.draw(this.blipPositionX-this.shakingPos.x,this.pos.y+25-this.shakingPos.y);},calculateShaking:function(){var rand=Math.random();if(ig.gameStatus==ig.gameStates.flying){if(rand<0.33){this.shakingPos.y=2;}else if(rand<0.66){this.shakingPos.y=1;}else{this.shakingPos.y=0}
if(Math.random()<0.5){this.shakingPos.y*=-1}
rand=Math.random();if(rand<0.33){this.shakingPos.x=2;}else if(rand<0.66){this.shakingPos.x=1;}else{this.shakingPos.x=0}
if(Math.random()<0.5){this.shakingPos.x*=-1}}else{if(rand<0.5){this.shakingPos.y=1;}else{this.shakingPos.y=0;}
if(rand<0.5){this.shakingPos.x=1;}else{this.shakingPos.x=0;}}},registerBlip:function(posX){this.blipPositionX=posX;},});});

// lib/game/entities/endingGame.js
ig.baked=true;ig.module('game.entities.endingGame').requires('impact.game','game.plugins.button').defines(function(){EntityEndingGame=EntityButton.extend({anim:null,zIndex:500,init:function(x,y,settings){console.log("EntitiyEndingGame spawned");this.parent(x,y,settings);var animSheet=new ig.AnimationSheet(settings.imageName,320,568);this.anim=new ig.Animation(animSheet,0.1,[0]);},update:function(){this.parent();},pressed:function(){this.parent();ig.gameStatus=ig.gameStates.menu;this.kill();},draw:function(){this.anim.draw(this.pos.x,this.pos.y);}});});

// lib/game/entities/menu.js
ig.baked=true;ig.module('game.entities.menu').requires('impact.game','game.plugins.button').defines(function(){EntityMenu=EntityButton.extend({anim:null,zIndex:500,init:function(x,y,settings){console.log("EntityMenu spawned");this.parent(x,y,settings);var animSheet=new ig.AnimationSheet(settings.imageName,320,568);this.anim=new ig.Animation(animSheet,0.1,[0]);},update:function(){this.parent();},pressed:function(){this.parent();ig.gameStatus=ig.gameStates.launchPrep;ig.launchTimer.reset();this.kill();},draw:function(){this.anim.draw(this.pos.x,this.pos.y);}});});

// lib/game/entities/fuelRocket.js
ig.baked=true;ig.module('game.entities.fuelRocket').requires('impact.game').defines(function(){EntityFuelRocket=ig.Entity.extend({animRocket:null,size:{x:13,y:60},type:ig.Entity.TYPE.A,init:function(x,y,settings){this.parent(x,y,settings);var animSheet=new ig.AnimationSheet('media/fuelRocket.png',13,60);this.animRocket=new ig.Animation(animSheet,0.1,[0]);},update:function(){this.parent();},draw:function(){this.animRocket.draw(this.pos.x,this.pos.y);}});});

// lib/game/entities/asteroid.js
ig.baked=true;ig.module('game.entities.asteroid').requires('impact.game').defines(function(){EntityAsteroid=ig.Entity.extend({animAsteroid:null,size:{x:25,y:68},type:ig.Entity.TYPE.B,maxVel:{x:500,y:500},init:function(x,y,settings){this.parent(x,y,settings);var animSheet=new ig.AnimationSheet('media/asteroid.png',25,68);this.animAsteroid=new ig.Animation(animSheet,0.1,[0]);},update:function(){this.parent();if(this.pos.y>570){this.kill();}},draw:function(){this.animAsteroid.draw(this.pos.x,this.pos.y);}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','game.entities.booster','game.entities.rocket','game.entities.rollingBackground','game.entities.messageManager','game.entities.radar','game.entities.endingGame','game.entities.menu','game.entities.fuelRocket','game.entities.asteroid').defines(function(){RocketGame=ig.Game.extend({currentFuelRocketTime:18.25,fuelRocketTimer:new ig.Timer(30),fuelRocketTimerIncrease:3.75,nextAsteroidTimer:25,asteroidTimer:new ig.Timer(this.nextAsteroidTimer),asteroidSpeedCoefficient:4,asteroidSpawningRate:2.5,rocketBoosterSoundTimer:new ig.Timer(6),rocket:undefined,gameSkippingTimer:new ig.Timer(1),init:function(){ig.input.bind(ig.KEY.MOUSE1,'MousePressed');ig.input.bind(ig.KEY.LEFT_ARROW,'LeftArrow');ig.input.bind(ig.KEY.RIGHT_ARROW,'RightArrow');ig.game.spawnEntity(EntityBooster,0,468,{rocketDirection:"left",imageName:"media/boosterRight.png",touchName:"MousePressed",keyName:"LeftArrow",size:{x:100,y:100}});ig.game.spawnEntity(EntityBooster,220,468,{rocketDirection:"right",imageName:"media/boosterLeft.png",touchName:"MousePressed",keyName:"RightArrow",size:{x:100,y:100}});ig.gameStates={launchPrep:"launchPrep",launchRocket:"launchRocket",menu:"menu",gameEnded:"gameEnded",waitingUserInput:"waitingUserInput",flying:"flying",gameSkippingToMenu:"gameSkippingToMenu"};ig.rocketDirection=false;ig.rocketVerticalSpeed=0;ig.rocketVerticalAcceleration=10;ig.timeTillLaunch=3;ig.timeTillLaunchOriginal=3;ig.gameStatus=ig.gameStates.menu;ig.launchTimer=new ig.Timer(3.1);this.rocket=ig.game.spawnEntity(EntityRocket,140,468,{imageName:"media/rocket.png"});ig.game.spawnEntity(EntityRollingBackground,-220,-9282,{name:"rollingBackground",imageName:"media/launchPad.png"});ig.game.spawnEntity(EntityRadar,-240,0,{imageName:"media/radarGrid.png"});ig.game.spawnEntity(EntityMessageManager,0,0);ig.game.sortEntitiesDeferred();ig.rocketBoosterSound=new ig.Sound('media/sounds/engineRunning.mp3');ig.explosionSound=new ig.Sound('media/sounds/explosion.mp3');},update:function(){this.parent();if(ig.gameStatus==ig.gameStates.launchPrep){if(ig.launchTimer.delta()>0){ig.gameStatus=ig.gameStates.launchRocket;this.rocketBoosterSoundTimer.set(5.5);ig.rocketBoosterSound.play();this.fuelRocketTimer.reset();this.asteroidTimer.reset();}else if(ig.launchTimer.delta()<=0){ig.timeTillLaunch=Math.floor(Math.abs(ig.launchTimer.delta()));}}
if(ig.gameStatus==ig.gameStates.menu){this.reboot();ig.game.spawnEntity(EntityMenu,0,0,{imageName:"media/startImage.png",touchName:"MousePressed",size:{x:320,y:568}});ig.gameStatus=ig.gameStates.waitingUserInput;}
if(ig.gameStatus==ig.gameStates.gameEnded){ig.gameStatus=ig.gameStates.gameSkippingToMenu;this.gameSkippingTimer.reset();this.gameEndScreen=ig.game.spawnEntity(EntityEndingGame,0,0,{imageName:"media/endImage.png",touchName:"MP",size:{x:320,y:528}});}
if(ig.gameStatus==ig.gameStates.gameSkippingToMenu&&this.gameSkippingTimer.delta()>0){ig.gameStatus=ig.gameStates.menu;this.gameEndScreen.kill();}
if(ig.gameStatus==ig.gameStates.flying){this.calculateFuelRocket();this.calculateAsteroid();this.checkBoosterSound();}},draw:function(){this.parent();},checkBoosterSound:function(){if(this.rocketBoosterSoundTimer.delta()>0){this.rocketBoosterSoundTimer.set(6);ig.rocketBoosterSound.play();}},reboot:function(){console.log("main.reboot invoked");ig.rocketDirection=false;ig.rocketVerticalSpeed=0;ig.rocketVerticalAcceleration=10;ig.timeTillLaunch=3;ig.timeTillLaunchOriginal=3;ig.game.getEntityByName("rollingBackground").reboot();this.rocket.pos.x=140;this.rocket.pos.y=468;this.asteroidSpeedCoefficient=4;},calculateFuelRocket:function(){if(this.fuelRocketTimer.delta()>=0){this.fuelRocketTimer.set(this.currentFuelRocketTime+this.fuelRocketTimerIncrease);rollingBackground=ig.game.getEntitiesByType("EntityRollingBackground")[0];ig.game.spawnEntity(EntityFuelRocket,Math.floor(Math.random()*220+50),-60,{accel:rollingBackground.accel,vel:rollingBackground.vel});this.increaseGameDifficutly();ig.game.getEntitiesByType(EntityMessageManager)[0].pushMessage("Collect fuel rocket to refuel...",5);}},increaseGameDifficutly:function(){this.asteroidSpeedCoefficient++;if(this.asteroidSpawningRate<0){this.asteroidSpawningRate=0;}else{this.asteroidSpawningRate-=0.5;}},calculateAsteroid:function(){if(this.asteroidTimer.delta()>=0){this.asteroidTimer.set(Math.floor(Math.random()*1+this.asteroidSpawningRate));var entry=Math.floor(Math.random()*300+10);ig.game.spawnEntity(EntityAsteroid,entry,-250,{vel:{x:0,y:200+50*Math.floor(Math.random()*this.asteroidSpeedCoefficient)}});ig.game.getEntitiesByType("EntityRadar")[0].registerBlip(entry+Math.floor(Math.random()*20-10));}}});ig.main('#canvas',RocketGame,60,320,568,1);});